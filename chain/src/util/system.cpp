...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string datadir_before_conf = GetArg("-datadir", "");
    ...
    if (!CheckDataDirOption()) {
        const std::string datadir = GetArg("-datadir", "");
        if (!datadir.empty()) {
            TryCreateDirectories(fs::system_complete(datadir));
            gArgs.ClearPathCache();
        }
    }
    if (!CheckDataDirOption()) {
        fs::path fallback;
        if (!datadir_before_conf.empty()) {
            fallback = fs::system_complete(datadir_before_conf);
        } else {
            fallback = GetDefaultDataDir();
        }
        TryCreateDirectories(fallback);
        TryCreateDirectories(fallback / "wallets");
        ForceSetArg("-datadir", fallback.string());
        ClearPathCache();
        if (!CheckDataDirOption()) {
        ...
        LogPrintf(
            "Warning: ignored invalid datadir in configuration file; using \"%s\" instead.\n",
            fallback.string());
    }
    return true;
}
