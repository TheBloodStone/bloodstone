...
import { initAndroidAppUpdate } from "./app-update.js";
import {
    ...
    if (isAndroidAppContext()) {
      ...
      if (shouldHostLocalNode(nodeMode)) {
        nodeStatus = await startLocalNode({
          nodeMode,
          foreground: nodeMode === NODE_MODES.FULL,
          waitForStratumMs: nodeMode === NODE_MODES.FULL ? 6000 : 2000,
        });
...
function stopMining(userInitiated = true, options = {}) {
  ...
  void stopFleetNode();
  if (isAndroidAppContext() && stopNode) {
