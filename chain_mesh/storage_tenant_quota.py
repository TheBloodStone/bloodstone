"""Wave R — multi-tenant storage quota (per-author byte caps on shared Pi / STONE pools)."""
...
TENANT_FORMAT = "bloodstone_storage_tenant/v1"


def _tenant_enable() -> bool:
    return os.environ.get("STORAGE_TENANT_ENFORCE", "1").strip().lower() not in (
        "0",
        "false",
        "no",
    )
...
def _default_author_cap() -> int:
    return max(0, int(os.environ.get("STORAGE_TENANT_AUTHOR_CAP", "0")))
...
def tenant_quota(
    *,
    ...
    stone_q = sc.quota_summary(bound_stone) if bound_stone else {}
...
def check_tenant_storage_allowed(
    *,
    stone_address: str,
    byte_size: int = 0,
    blurt_account: str = "",
    ...
    if sc.ENFORCE_QUOTA and stone_address and int(stone_q.get("bytes_remaining") or 0) < max(
        0, int(byte_size)
    ):
        return {
            "ok": True,
            "allowed": False,
            "quota": stone_q,
            "tenant": {"skipped": True, "reason": "stone storage quota denied"},
    ...
    return {
        ...
        "quota": stone_q,
        "tenant": tq,
        "reason": f"tenant author storage cap exceeded: need {need} bytes, have {remaining} for @{author}",
    }
...
def status_payload() -> Dict[str, Any]:
    ...
    with _conn() as conn:
        authors = conn.execute(
            "SELECT COUNT(*) AS c FROM storage_tenant_bindings"
        ).fetchone()["c"]
        usage_rows = conn.execute(
            "SELECT COUNT(*) AS c FROM storage_tenant_usage"
        ).fetchone()["c"]
    ...
    return {
        ...
        "apis": {
            "quota": f"{public}/api/convergence/storage/tenant/quota",
            "bind": f"{public}/api/convergence/storage/tenant/bind",
            "status": f"{public}/api/convergence/storage/tenant/status",
        },
    }