...
def resolve_public_peers(
    ...
    *,
    ...
    return _split_peers(
        ...
        # Fallback: public seed host + coin p2p port (Fork Lab children)
        (
            f"64.188.22.190:{int(coin.get('p2p_port') or meta.get('p2p_port') or 0)}"
            if int(coin.get("p2p_port") or meta.get("p2p_port") or 0) > 0
            else ""
        ),
    )

_CACHE: Dict[str, Any] = {"at": 0.0, "coins": []}
_CACHE_TTL = float(os.environ.get("MFQ_CATALOG_TTL", "20"))
    ...
    for coin in _from_fork_lab():
        ...
        if t in by_ticker:
            ...
            for key in (
                ...
                if coin.get(key):
                    base[key] = coin[key]
            # Reconcile seed list after merge
