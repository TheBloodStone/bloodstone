...
function loadSettings(dataDir) {
  ...
  settings.addnodes = resolveSeedNodes(settings);
  settings.addnode = settings.addnodes[0] || DEFAULT_SEED;
...
function resolveSeedNodes(settings) {
  const fromList = Array.isArray(settings?.addnodes)
    ? settings.addnodes.map((seed) => String(seed || "").trim()).filter(Boolean)
    : [];
  if (fromList.length) return [...new Set(fromList)];
  const single = String(settings?.addnode || DEFAULT_SEED).trim();
  return single ? [single, ...DEFAULT_SEEDS.filter((seed) => seed !== single)] : [...DEFAULT_SEEDS];
}

function buildConfContent(settings) {
  const seedNodes = resolveSeedNodes(settings);
  const lines = [
    ...
    "chain=main",
    ...seedNodes.map((seed) => `addnode=${seed}`),
    "server=1",
    "txindex=1",
    "daemon=0",
    "listen=1",
