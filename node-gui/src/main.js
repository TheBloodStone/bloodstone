...
function registerIpc() {
  ...
  ipcMain.handle("node:reset-chain-data", async () => {
    ...
    const choice = await dialog.showMessageBox(mainWindow, {
      ...
      message: "Delete local blockchain data and resync from the network?",
      detail:
  ...
  ipcMain.handle("wallet:unlock", async (_evt, { passphrase, seconds = 1800 }) => {
    ...
    try {
      const wallet = activeWalletForUser(walletSession);
      await walletRpc.unlockWallet(wallet, passphrase, seconds);
      return { ok: true, message: `Wallet unlocked for ${Math.round(seconds / 60)} minutes` };
    } catch (err) {
      return { ok: false, message: err.message || String(err) };
    }
  });
