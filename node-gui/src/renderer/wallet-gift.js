...
const giftEls = {
  ...
  poolBalance: $g("#gift-pool-balance"),
...
async function refreshGift() {
  ...
  const pool = statusResult.pool || {};
  if (giftEls.poolBalance) {
    giftEls.poolBalance.textContent = `${formatGiftAmount(pool.spendable)} STONE`;
  }
  if (giftEls.activeEscrow) {
    giftEls.activeEscrow.textContent = `${formatGiftAmount(pool.active_escrow)} STONE`;
  }
