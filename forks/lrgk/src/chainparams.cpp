...
constexpr const char pszTimestampMainnet[]
    = "22/Jun/2026: LRGK independent chain salt=cc7779f7600b0402";
...
public:
    CMainParams() {
        ...
        consensus.initialSubsidy = 500 * COIN; // LRGK independent chain era-0 reward // LRGK independent chain era-0 reward // Year-1 base (stepped issuance)
        // Jul 2026 policy: no mid-year-1 jump at 12000 — full year at 100, then Y2–3 at 1000.
        consensus.nIncreasedSubsidyHeight = 0; // LRGK: no mid-chain subsidy jump // LRGK: no mid-chain subsidy jump
...
// LRGK side-chain bootstrap: easy powLimit so genesis (nBits 0x207fffff) and early mining are viable on CPU.
        ...
        // === LRGK independent genesis (pre-mined, not Bloodstone) ===
        // Premine pays real P2PKH (LSvzemvYGNLtaS2qF2i7x2CvDAsehMTq7s) — rule fork-premine-v1-real-key-only
        genesis = CreateGenesisBlockP2PKH(
            1785217589 /*nTime*/,
            3 /*nNonce*/,
            0x207fffff /*nBits*/,
            "15/Jul/2026: LRGK independent chain salt=cc7779f7600b0402",
            uint160S("548beddd8685b355c3c4427632d8722e2449f806"));
        consensus.hashGenesisBlock = uint256S("0xc251baf97b7b677312f10bf87bd4e0d06034c74275f7c7fd0c2758cdda20ebeb");
        assert(genesis.GetHash() == consensus.hashGenesisBlock);
        assert(genesis.hashMerkleRoot == uint256S("0xc46e11c49078632f3291f3e8574095665c4f9807569a3eb16c959fcffbf6f697"));
        assert(BlockMerkleRoot(genesis) == genesis.hashMerkleRoot);

        // LRGK: no Bloodstone seeds — independent network
        // vSeeds.emplace_back("seed.lrgk.example:33685");
        // (removed Bloodstone seed — LRGK independent)

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,48); // LRGK L…
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,125);
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,191); // LRGK WIF
