...
async function nativeStratumAvailable() {
  ...
  const cap = await whenStratumPluginReady(12000);
  return Boolean(cap?.Plugins?.BloodstoneStratum);
}
...
export async function createStratumTransport(poolKey, options = {}) {
  const androidApp = isAndroidAppContext();
  const useNative = await nativeStratumAvailable();
  if (androidApp && !useNative) {
    throw new Error(
      "Native stratum plugin not ready — close and reopen the app, then try again",
    );
  }
