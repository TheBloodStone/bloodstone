...
class NodeManager {
  constructor(resourcesPath) {
    ...
    this._genesisMismatch = false;
  }

  hasGenesisMismatch() {
    return this._genesisMismatch;
  }

  _handleDaemonLine(line) {
    if (!lineIndicatesGenesisMismatch(line)) {
      return;
    }
    this._genesisMismatch = true;
    this._emitLog(
      "[gui] Genesis mismatch — old SpaceXpanse chain data or wrong data folder. Use Reset chain data (wallets are kept)."
    );
    this._emitStatus(genesisMismatchStatus({
      running: this.isNodeActive(),
      processManaged: this.isRunning(),
      rpcReachable: this._rpcReachable,
    }));
  }
  ...
  beginMonitoring() {
    ...
    if (
      !this._genesisMismatch &&
      scanDebugLogForGenesisMismatch(this.settings.dataDir)
    ) {
      this._genesisMismatch = true;
      this._emitStatus(genesisMismatchStatus());
    }
    ...
    return new Promise((resolve) => {
      ...
      this.process.on("exit", (code, signal) => {
        ...
        if (!settled) {
          finish({
            ok: false,
            message: this._genesisMismatch
              ? `${GENESIS_MISMATCH_MESSAGE} Open Dashboard → Reset chain data.`
              : `Node exited immediately (code=${code ?? "?"}). Open the Logs tab for details.`,
          });
        }
      });
